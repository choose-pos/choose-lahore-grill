...
import { DateTimePicker } from "@/components/ui/form-date-picker";
...
const EventForm: React.FC<FormProps> = ({ isEvent = false }) => {
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await fetch(
        isEvent ? "/api/event-inquiry" : "/api/reservation-inquiry",
        {
          method: "POST",
          headers: {
      ...
      {isSubmitted ? (
        ...
        >
          <p className="text-center font-secondary text-4xl lg:text-[60px] xl:text-[90px] mb-5 lg:mb-20 text-bg1">
            {isEvent ? "Host an Event." : "Reserve Your Table Today"}
          </p>
